...
| `vaults_malformed` | The indexer answered, but described one or more escrowed vaults in a shape this bot cannot act on — a non-hex `vaultId`, or a `btcAmount`/`currentDebt` that is not a decimal integer string. Those entries are dropped and the rest of the cycle proceeds. Sustained, it means the indexer's wire format has changed and those vaults are never acquired |
