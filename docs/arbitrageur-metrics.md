...
| `vaults_malformed` | The indexer answered, but described one or more escrowed vaults in a shape this bot cannot act on — a `vaultId` that is not 32 bytes of hex, or a `btcAmount`/`currentDebt` that is not a decimal integer string. Those entries are dropped and the rest of the cycle proceeds. Sustained, it means the indexer's wire format has changed and those vaults are never acquired |
