...
async function start(routes: readonly ControlRoute[], host = "127.0.0.1"): Promise<string> {
  const server = startControlServer({ port: 0, host, routes, logger: silentLogger });
  servers.push(server);
...
describe("startControlServer", () => {
  ...
  it("consults every route until one handles the request", async () => {
    const declined = vi.fn<ControlRoute>(() => false);
    const base = await start([declined, haltRoute]);
  ...
  it("warns when bound to 0.0.0.0", async () => {
    ...
    expect(silentLogger.warn).toHaveBeenCalledWith(expect.stringContaining("0.0.0.0"));
  });
});
